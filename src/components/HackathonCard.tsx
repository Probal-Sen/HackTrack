...
import { CalendarDays, Clock, CheckCircle, XCircle, Edit, Trash, Check, X } from 'lucide-react';
...
const HackathonCard: React.FC<HackathonCardProps> = ({ hackathon }) => {
  const { toggleHackathonStatus, updateExistingHackathon, removeHackathon } = useHackathons();
  ...
  const isUpcoming = !hackathon.completed && new Date(hackathon.date) > new Date();
      ...
      <div className="p-5">
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <h3 className={`text-xl font-bold ${hackathon.completed ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
              {hackathon.name}
            </h3>
            ...
            <div className="mt-2">
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                {hackathon.completed ? 'Completed' : isUpcoming ? 'Upcoming' : 'Past due'}
              </span>
            </div>
          </div>
          ...
          <div className="flex space-x-2">
            <button 
              ...
              title={hackathon.completed ? 'Mark as incomplete' : 'Mark as complete'}
            >
              {hackathon.completed 
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-sm font-medium text-gray-700">Notes & Memories</h4>
            {!isEditing && (
              <button 
          ...
          {isEditing ? (
            ...
            <p className="text-gray-600 text-sm">
              {hackathon.notes 
                ? hackathon.notes 
                : <span className="italic text-gray-400">No notes added yet. Click the edit button to add some memories!</span>
              }
            </p>
          )}
        </div>
